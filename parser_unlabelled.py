...
#Loading word embeddings, pos_embeddings
...
outputs = layers.Dense(3, name="predictions")(x1)
...
for epoch in range(epochs):
    ...
    for sent_index, sentence in enumerate(data_train):
        ...
        while (len(stack)>1 or len(buffer)>0):
            correct_act = utilities.correct_action_unlabelled(stack, buffer)
            ...
            acc_metric.update_state(tf.math.argmax(correct_act[0]), tf.math.argmax(predicted_act[0]))
            if (correct_act == shift).all():
                stack.append(buffer[0])
                buffer.pop(0)
            elif (correct_act == left_arc).all():
                stack.pop(-2)
            else:
                stack.pop(-1)
                   
        if sent_index % 50 == 49:
            print(f'Training accuracy at epoch {epoch+1} sentence {sent_index+1}: {acc_metric.result()}')

    print(f'Accuracy at epoch {epoch+1}: {acc_metric.result()}')
...
for sent_index, sentence in enumerate(data_dev):
    ...
    if sent_index % 50 == 49:
        print(f'Dev accuracy at sentence {sent_index+1}: {acc_metric.result():.5f}')

print(f'Accuracy on entire dev set: {acc_metric.result()}')
acc_metric.reset_states()
...
for sent_index, sentence in enumerate(data_test):
    ...
    if sent_index % 50 == 49:
        print(f'Test accuracy at sentence {sent_index+1}: {acc_metric.result():.5f}')

print(f'Accuracy on entire test set: {acc_metric.result()}')
acc_metric.reset_states()